...
void Vectors::Square(list<list<double>> &matrix, unsigned &componentnumber, bool complete){
	...
	if(matrix.size() == componentnumber)	return;
		...
		//Indicate failure through componentnumber
		componentnumber = -1;
	}else{
		list<double> filler;
...
double Vectors::Determinant(list<list<double>> &matrix, unsigned total_size){
	...
	if( matrix.size() != total_size)
		return -1;
	else{	//Calculate determinant
		double toreturn=0;
...
double Vectors::Magnitude(double vec[]){
	double toreturn = 0;
	for(size_t i(0); i<sizeof(vec)/sizeof(double);++i)
		toreturn += i*i;
	return toreturn;
}
...
double Vectors::Direction(double v[], int axis1, int axis2){
	if(axis1 < 1 || axis2 < 1)	return -1.0000;
...
double Vectors::Direction(const initializer_list<double>& v, int axis1, int axis2){
	if(axis1 < 1 || axis2 < 1)	return -1.00000;
	else if(axis1 == axis2)	return 0;
	vector<double> vec(v);
...
double Vectors::Dot(const double f[],const double s[]){
	using std::distance;
	using std::end;
	using std::begin;
	size_t
		fdist(sizeof(f)/sizeof(double)),
		sdist(sizeof(f)/sizeof(double))
	;
	bool firstlarger = (fdist>sdist);
	size_t newsize = firstlarger ? fdist : sdist;
	double first[newsize], second[newsize], toreturn(0);
	for(size_t i(0); i < (firstlarger ? sdist : fdist); ++i){
		first[i] = f[i];
		second[i] = s[i];
	}
	for(size_t i((firstlarger ? sdist : fdist)-1);
		i < sdist-fdist;
		++i
	) (firstlarger ? second[i] : first[i]) = 0.0;
	for(size_t i(0); i < newsize; ++i)
		toreturn += first[i]*second[i];
	return toreturn;
}
...
Vectors::VectorData Vectors::Cross(const initializer_list<initializer_list<double>>& veclist){
	...
	{	//Limit scope further to encapsulate temporary variables
		...
		for(column=0, sign = 1; column < vectorlist.begin()->size(); column++, sign *= -1){
			...
			if(coefficient < epsilon__ && coefficient > epsilon__)	toreturn.push_back(0);
...
double Vectors::VectorData::Direction(int index)const{
	...
	if(index < 0 || index > convert)	return Zero__;
...
double& Vectors::VectorData::operator[](size_t dimension)const{
	static double hold;
		hold =
            (dimension < 0 || dimension > components.size()-1) ?
            Zero__ :
